/*
  ...
  1. New Tables
    - `conversations` (for both case-related and general chats)
    - `conversation_participants` (tracks who can access each conversation)
    - `messages` (stores all chat messages)

  2. Security
    - Enable RLS on all tables
    - Add helper function to prevent recursive policy issues
    - Add policies for conversation access control
...
-- Create helper function to check conversation access (prevents RLS recursion)
...
-- Conversations policies (using helper function to prevent recursion)
...
-- Messages policies (using helper function to prevent recursion)
